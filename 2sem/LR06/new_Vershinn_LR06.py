...
class SolverCauchyFirstOrder:
    def __init__(self, win_frame) -> None:
        ...
        self.__plot_odeint_solution()

    def __plot_odeint_solution(self):
        # создадим функцию
        def dydt(y, t):
            return 0.5*np.sin(2*t)-y*np.cos(t)

        t = np.linspace(0, 1, 51) # вектор Х
        yn=np.sin(t)-1+np.exp(-np.sin(t))
        y0 = 0 # начальное значение
        y = odeint(dydt, y0, t) # решение диф.уравн
        y = np.array(y).flatten()
        ye = self.__euler_method()
        ye_res = []
        for i in range(len(ye[1])):
            ye_res.append(1 - ye[1][i])

        
        fig = Figure(figsize=(4, 4), facecolor='peachpuff')
        ax = fig.add_subplot(facecolor='bisque')
        ax.plot(t, yn, 'green', linewidth=4, label='Точное решение') # график исходной функции F(x)
        ax.plot(t, y, '*r', linewidth=2, label='Рунге-Крутта') # полученный график
        ax.plot(ye[0], ye_res, 'red', linewidth=2, label='Эйлер')
        ax.grid(which='major', axis='y', color='gray', linestyle='-.', linewidth=0.5)
        
        canvas = FigureCanvasTkAgg(fig, master=self.win_frame)
        canvas.draw()
        canvas.get_tk_widget().place(x=400, y=300)
    ...
    def __tkinter_fun_scfo(self) -> None:
        ...
        Button(self.win_frame, text="Решить", font="14", bg='bisque2', command=lambda: disp_info(self, 0, 700)).place(x=30, y=220)
...
class SolverCauchySysFirstOrder:
    ...
    def __plot_odeint_solution(self):
        # создадим функцию для системы
        def dydt(y, t):
            return [-y[0] + 8*y[1], y[0] + y[1]]

        t = np.linspace(0, 1, 51) # вектор Х
        y0 = [self.y10, self.y20] # начальные значения
        y = odeint(dydt, y0, t) # решение системы диф. уравнений
        
        fig1 = Figure(figsize=(4, 4), facecolor='peachpuff')
        ax1 = fig1.add_subplot(facecolor='bisque')
        ax1.plot(t, y[:,0], 'green', linewidth=4, label='y1 точное')
        ax1.plot(t, y[:,0], '*r', linewidth=2, label='y1 решение')
        ...
        ax2.plot(t, y[:,1], 'green', linewidth=4, label='y2 точное')
        ax2.plot(t, y[:,1], '*r', linewidth=2, label='y2 решение')
        ax2.grid(which='major', axis='y', color='gray', linestyle='-.', linewidth=0.5)
        ax2.legend()
        
        canvas1 = FigureCanvasTkAgg(fig1, master=self.win_frame)
        canvas1.draw()
        canvas1.get_tk_widget().place(x=400, y=300)
        
        canvas2 = FigureCanvasTkAgg(fig2, master=self.win_frame)
        canvas2.draw()
        canvas2.get_tk_widget().place(x=400, y=600)
        
        self.widgets.extend([canvas1, canvas2])
    ...
    def __tkinter_fun_scsfo(self) -> None:
        ...
        Button(self.win_frame, text="Решить", font="14", bg='bisque2', command=lambda: disp_info(self, 450, 200)).place(x=30, y=220)
...
class SolverCauchySecondOrder:
    ...
    def __plot_odeint_solution(self):
        # создадим функцию для уравнения второго порядка
        def dydt(y, t):
            return [y[1], y[0] + 2*np.exp(t) - t**2]

        t = np.linspace(0, 1, 51) # вектор Х
        y0 = [self.y0, self.y_prime0] # начальные значения
        y = odeint(dydt, y0, t) # решение диф. уравнения второго порядка
        ye = self.__euler_method()
        ye_res = []
        for i in range(len(ye[1])):
            ye_res.append(ye[1][i])

        
        fig = Figure(figsize=(4, 4), facecolor='peachpuff')
        ax = fig.add_subplot(facecolor='bisque')
        ax.plot(t, y[:,0], 'green', linewidth=4, label='Точное решение')
        ax.plot(t, y[:,0], '*r', linewidth=2, label='Рунге-крутта')
        ax.plot(ye[0], ye_res, 'red', linewidth=2, label='Метод эйлера')
        ax.grid(which='major', axis='y', color='gray', linestyle='-.', linewidth=0.5)
        ax.legend()
        
        canvas = FigureCanvasTkAgg(fig, master=self.win_frame)
        canvas.draw()
        canvas.get_tk_widget().place(x=-1, y=300)
        self.widgets.append(canvas)
